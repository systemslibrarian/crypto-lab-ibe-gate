 ...
 *    files the node under `incomplete`. This lab paints no gradients, so that
 *    half finds nothing here; the rule is kept because it costs nothing and the
 *    palette is `color-mix`-based, one `color-mix` away from the same problem.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. THIS is the half that
 *    mattered here, and it is exactly what the gate this replaces destroyed by
 *    injecting `opacity: 1 !important` before every scan. Four separate defects
 *    lived under that injection: the hero subtitle at `.85` on top of an already
 *    muted token (4.45:1), the protocol diagram's arrow labels at `.5` (4.06:1
 *    dark, 2.12:1 light), and the loading spinner, which the stylesheet faded to
 *    `.6` *inside* `prefers-reduced-motion` — so it was 2.76:1 only for readers
 *    who asked not to be flashed at, and only while they were waiting.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.gt-hex-dump` is
 *    a `max-height: 220px` scroller holding all 576 bytes of a G_T element, so
 *    most of those byte spans sit outside its client box at any moment. Content
 *    scrolled past that box is not dimmed or partly drawn — it is absent from
 *    the frame, and asking what colour it sits on has no answer. Its rect is
 *    still outside every ancestor's box, so the ancestor walk would find nothing
 *    behind it and fall through to white, inventing a failure. Skip it, and rely
 *    on the bytes that ARE in view, which are measured for real.
 *
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This is
 *    load-bearing here. `#flow-svg` draws each actor as a `<rect>` and then its
 *    labels as following `<text>` nodes, so a label's backdrop is a PRECEDING
 *    SIBLING shape, not an ancestor. An ancestor-only walk would composite those
 *    labels onto the svg's own transparent background and report a ratio nothing
 *    on screen has. It is what found `.actor-role` reading "the shared mask" at
 *    3.97:1 — that text sits inside the cyan-tinted G_T node, not on the page,
 *    and against the page it measures fine. Only shapes that actually paint a
 *    fill count: SVG's initial `fill` is black and `getComputedStyle` reports
 *    that for stroke-only geometry too, so a `<line>` used as a connector would
 *    otherwise read as an opaque black rectangle — and this diagram's five
 *    arrows are exactly that: stroked `<path>`s with `fill: none`.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * This page renders all 576 bytes of a G_T element as 576 individually
     * coloured spans in the inspector, plus a byte grid for the XOR mask, and
     * every one of them walks the same handful of ancestors. Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab leans on <details> directly: the 576-byte G_T
      // inspector is one, and the gate opens it and scans the revealed state
      // explicitly rather than measuring it shut.
     ...
     * it, the `aria-hidden` decoration this lab carries — the `.op-symbol` XOR
     * glyphs, the colour-coded "Alice decrypts / Eve decrypts" column headings
     * that repeat what the terminals below them already say, and the `.censor`
     * redaction bars drawn as red block characters on red — would be measured as
     * if each were text meant to be read. The redaction bars are the clearest
     * case: they are 1:1 BY DESIGN, and reporting that is reporting the feature.
     */
